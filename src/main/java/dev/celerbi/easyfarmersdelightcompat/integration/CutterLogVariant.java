...
/** Canonical material-variant encoding for the Cutter. */
public final class CutterLogVariant {
    ...
    public static boolean isAllowed(ItemStack stack) { return stack != null && !stack.isEmpty() && stack.is(ALLOWED_LOGS) && stack.getItem() instanceof BlockItem; }
    public static Block fromIngredient(ItemStack stack) { return isAllowed(stack) && stack.getItem() instanceof BlockItem bi ? bi.getBlock() : Blocks.OAK_LOG; }
    public static Block fromStack(ItemStack stack) {
        if (stack == null || stack.isEmpty()) return Blocks.OAK_LOG;
        CustomData data = stack.get(DataComponents.BLOCK_ENTITY_DATA);
        return data == null ? Blocks.OAK_LOG : read(data.copyTag());
    }
    public static Block read(CompoundTag tag) {
        if (tag == null || !tag.contains(NBT_KEY)) return Blocks.OAK_LOG;
        ResourceLocation id = ResourceLocation.tryParse(tag.getString(NBT_KEY));
        if (id == null) return Blocks.OAK_LOG;
    ...
    public static void write(CompoundTag tag, Block log) {
        if (tag == null || log == null || log == Blocks.OAK_LOG || !isAllowed(new ItemStack(log.asItem()))) return;
    ...
    public static ItemStack createCutter(Block log) {
        ...
        if (log == null || log == Blocks.OAK_LOG) return result;
        CompoundTag tag = new CompoundTag(); write(tag, log);
        if (!tag.isEmpty()) BlockItem.setBlockEntityData(result, ModBlockEntities.CUTTER.get(), tag);
        return result;
    }
    public static String translationKey(Block log) {
        String name = log == Blocks.SPRUCE_LOG ? "spruce" : log == Blocks.BIRCH_LOG ? "birch" : log == Blocks.JUNGLE_LOG ? "jungle"
                : log == Blocks.ACACIA_LOG ? "acacia" : log == Blocks.DARK_OAK_LOG ? "dark_oak" : log == Blocks.MANGROVE_LOG ? "mangrove"
