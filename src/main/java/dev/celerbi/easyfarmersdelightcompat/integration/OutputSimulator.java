...
/** Lossless output helper: simulate the complete operation before mutating inventory. */
public final class OutputSimulator {
    ...
    public static boolean canFitAll(IItemHandler output, List<ItemStack> stacks) {
        ...
        for (int slot = 0; slot < simulated.length; slot++) simulated[slot] = output.getStackInSlot(slot).copy();
    ...
    public static boolean insertAll(IItemHandler output, List<ItemStack> stacks) {
        if (!canFitAll(output, stacks)) return false;
        for (ItemStack source : stacks) {
            if (source == null || source.isEmpty()) continue;
            ItemStack remaining = source.copy();
            ...
            if (!remaining.isEmpty()) throw new IllegalStateException("Output changed after successful Cutter simulation");
        }
        return true;
    }
    ...
    private static void insertIntoSimulation(IItemHandler output, ItemStack[] simulated, ItemStack remaining) {
        for (int slot = 0; slot < simulated.length && !remaining.isEmpty(); slot++) {
            if (!output.isItemValid(slot, remaining)) continue;
    ...
    public static boolean canFitAll(Container output, List<ItemStack> stacks) {
        ...
        for (int slot = 0; slot < simulated.length; slot++) simulated[slot] = output.getItem(slot).copy();
        for (ItemStack source : stacks) {
            if (source == null || source.isEmpty()) continue;
            ItemStack remaining = source.copy();
            for (int slot = 0; slot < simulated.length && !remaining.isEmpty(); slot++) {
                if (!output.canPlaceItem(slot, remaining)) continue;
                ItemStack existing = simulated[slot];
                if (existing.isEmpty()) {
                    int move = Math.min(remaining.getCount(), Math.min(remaining.getMaxStackSize(), output.getMaxStackSize(remaining)));
                    simulated[slot] = remaining.copyWithCount(move);
                    remaining.shrink(move);
