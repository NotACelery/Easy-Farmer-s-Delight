...
public final class CutterBlockEntityRenderer implements BlockEntityRenderer<CutterBlockEntity> {
    ...
    private static final TagKey<Item> FLAT=TagKey.create(Registries.ITEM,ResourceLocation.fromNamespaceAndPath("farmersdelight","flat_on_cutting_board"));
    ...
    private void renderWorkstation(CutterBlockEntity cutter,Direction facing,PoseStack pose,MultiBufferSource buffer,int light,int overlay){pose.pushPose();applyWorkTransform(pose,facing);blockRenderer.renderSingleBlock(cutter.logVariant().defaultBlockState(),pose,buffer,light,overlay);Block board=BuiltInRegistries.BLOCK.get(CUTTING_BOARD_ID);if(board!=Blocks.AIR){pose.pushPose();pose.translate(0D,1D,0D);blockRenderer.renderSingleBlock(board.defaultBlockState(),pose,buffer,light,overlay);renderItem(cutter,pose,buffer,light,overlay);pose.popPose();}pose.popPose();}
